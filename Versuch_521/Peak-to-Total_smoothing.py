...
plt.plot(unp.nominal_values(x), unp.nominal_values(y), marker='.',linestyle = 'None', markersize=1)
# plt.fill_between(unp.nominal_values(x[:xmin]),unp.nominal_values(y[:xmin]),np.zeros(len(unp.nominal_values(y[:xmin]))), alpha=.25, color='blue')
...
# plt.fill_between(unp.nominal_values(x[xmin:xmax]),unp.nominal_values(y[xmin:xmax]),np.zeros(len(unp.nominal_values(y[xmin:xmax]))), alpha=.25, color='orange')
...
x_error = np.array(unp.std_devs(x))
y_error = np.array(unp.std_devs(y))
f = interpolate.Rbf(x, y, smooth=500)
y = f(x)
x = unp.uarray(x,x_error)
y = unp.uarray(y,y_error)
total_counts = np.sum(y)
